...
class MobileRpcSession(private val callback: Callback) : MobileTcpClient.Callback {
    interface Callback {
    ...
    fun connect(route: CmuxRoute) {
        callback.onConnectionState("connecting", "${route.host}:${route.port}")
    ...
    fun request(method: String, params: JSONObject = JSONObject()): Int {
        ...
        val request = JSONObject()
            ...
            .put("method", method)
            .put("params", params)
        client.sendFrame(request.toString())
        return requestId
    }
